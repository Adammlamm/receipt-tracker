#!/bin/bash
set -e
echo "Applying: everything from script 28, plus the new people-picker overlay (replaces long chip lists)..."
mkdir -p "app/friend/[slug]" "app/api/friend/[slug]"
rm -rf "app/friend/[id]" "app/api/friend/[id]"

mkdir -p $(dirname 'components/PeoplePicker.tsx')
cat > 'components/PeoplePicker.tsx' << 'FILEEOF'
"use client";
...
npm test
echo "Now run: git add . && git commit -m \"Add people-picker overlay, receipt notes, full split options\" && git push"
